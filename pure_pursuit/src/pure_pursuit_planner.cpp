// pure_pursuit_planner.cpp
// ─────────────────────────────────────────────────────────────────────────────
// 플러그인 본체. 외부 변환 유틸 의존 없이 이 파일만으로 심볼이 모두 해결되도록 구성.
// - getXPoseTF2(단일/벡터) 를 로컬 static 함수로 포함
// - nav_core::BaseLocalPlanner 를 pluginlib 로 export
// ─────────────────────────────────────────────────────────────────────────────
...
// pluginlib export (nav_core)
PLUGINLIB_EXPORT_CLASS(pure_pursuit_local_planner::PurePursuitPlanner,
                       nav_core::BaseLocalPlanner)

namespace pure_pursuit_local_planner
{

// === Local TF2 helpers (외부 .cpp 없이 이 파일에서 해결) ====================
static bool getXPoseTF2(tf2_ros::Buffer& tf2_buffer,
                        ...
                        int index)
{
  ...
  const int idx = std::max(0, std::min(index, (int)global_plan.size()-1));
  const std::string source_frame = global_plan.front().header.frame_id;
  if (source_frame.empty() || target_frame.empty()) return false;

  geometry_msgs::TransformStamped tf;
  try {
    tf = tf2_buffer.lookupTransform(target_frame, source_frame,
                                    ros::Time(0), ros::Duration(0.2));
  } catch (const tf2::TransformException& ex) {
    ROS_WARN_STREAM_THROTTLE(1.0, "[getXPoseTF2(idx)] TF lookup failed: " << ex.what());
    return false;
  }

  try {
    tf2::doTransform(global_plan[idx], out, tf);
  } catch (const tf2::TransformException& ex) {
    ROS_WARN_STREAM_THROTTLE(1.0, "[getXPoseTF2(idx)] doTransform failed: " << ex.what());
    return false;
  }
                        ...
                        int limit)
{
  ...
  try {
    ...
    ROS_WARN_STREAM_THROTTLE(1.0, "[getXPoseTF2(vec)] TF lookup failed: " << ex.what());
    return false;
  }

  const size_t N = global_plan.size();
  const size_t M = (limit > 0) ? std::min((size_t)limit, N) : N;
  transformed_plan.reserve(M);

  for (size_t i = 0; i < M; ++i) {
    geometry_msgs::PoseStamped out;
    try {
      tf2::doTransform(global_plan[i], out, tf);
    } catch (const tf2::TransformException& ex) {
      ROS_WARN_STREAM_THROTTLE(1.0, "[getXPoseTF2(vec)] doTransform failed @ " << i << ": " << ex.what());
      return false;
    }
    out.header.stamp    = ros::Time::now();
    out.header.frame_id = target_frame;
    transformed_plan.push_back(out);
  }
  return !transformed_plan.empty();
}
// ============================================================================

// === 유틸: 선분 충돌 검사 (costmap 상에서 선분 따라 치역 스캔) ================
...
void PurePursuitPlanner::initialize(std::string name,
                                    tf2_ros::Buffer* tf,
...
bool PurePursuitPlanner::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
  ...
  if (!getXPoseTF2(*tf2_buffer_, global_plan_, global_frame, tmp_target, base_idx)) return false;
  ...
  // (4) target pose (world→global_frame)
  geometry_msgs::PoseStamped target_global;
  if (!getXPoseTF2(*tf2_buffer_, global_plan_, global_frame, target_global, target_idx)) return false;
  ...
  // 헤딩/옆치기 억제
  double v_scale_heading = std::exp(-3.0 * std::fabs(ang_err));
  double v_scale_lateral = std::exp(-8.0 * std::min(0.25, std::fabs(ly)));
...
void PurePursuitPlanner::publishLocalPlan(const std::vector<geometry_msgs::PoseStamped>& path) const
{
  nav_msgs::Path p;
...
bool PurePursuitPlanner::getGoalLocalCoordinates(std::vector<double>& local_xy,
...
void PurePursuitPlanner::setControls(const std::vector<double>& lookahead_xy,
...
double PurePursuitPlanner::getEuclideanDistance(double x1, double y1, double x2, double y2) const
{
