...
ros::ServiceClient deletemap_cmd_client; 
tetraDS_TCP::deletemap deletemap_cmd_service; 
...
bool Delete_Location(string strLocationName)
{
    bool bResult = false;
...
bool Delete_Map(string strMapName) 
{
    bool bResult = false;
    ...
    if(!strcmp(cSTX, m_cSTX) && !strcmp(cETX, m_cETX))
    {
        switch(HashCode(m_cCMD))
        {
            ...
            case HashCode("KILL"): //Mapping Mode or Navigation Mode Kill Service Call
                NodeKill();
                sprintf(Send_buffer, " MAPPING / NAV MODE EXIT "); 
                break;
            ...
            case HashCode("LSV"): // Save to Location data
                ...
                break;
            case HashCode("LDL"): // delete to Location data 
                Delete_Location(m_cPARAM[0]);
                break;
            case HashCode("MDL"): // delete to Map data 
...
int main(int argc, char* argv[])
{
    ...
    deletemap_cmd_client = client_h.serviceClient<tetraDS_TCP::deletemap>("deletemap_cmd"); 
