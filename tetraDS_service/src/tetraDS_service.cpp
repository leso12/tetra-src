...
void Docking_EXIT()
{
    ...
    _pFlag_Value.m_bFlag_dock= false;
...
void TebMarkers_Callback(const visualization_msgs::Marker::ConstPtr& msg)
{
    if(msg->ns == "ViaPoints")
    {
        ...
        if(m_iViaPoint_Index <= 1)
        {
            ...
            {
                if(_pDynamic_param.m_linear_vel >= 0.05)
                {
                    if(!m_flag_Dynamic_TebMarkers_major_update)
                    {
                        //Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_x", 0.05);
...
void Teblocalplan_Callback(const geometry_msgs::PoseArray::ConstPtr& msg)
{
    ...
    printf("m_dDelta_Value :  %.5f !!\n", m_dDelta_Value);

    if(_pFlag_Value.m_bCorneringFlag)
    {
        if(m_dDelta_Value >= 1.2)
        {
            if(!m_flag_Dynamic_Teblocalplan_major_update)
            {
                ...
                if(_pGoal_pose.goal_positionX != 0.0 || _pGoal_pose.goal_positionY != 0.0){
                    float m_distance= sqrt((_pTF_pose.poseTFx -_pGoal_pose.goal_positionX) * (_pTF_pose.poseTFx -_pGoal_pose.goal_positionX) + (_pTF_pose.poseTFy -_pGoal_pose.goal_positionY) * (_pTF_pose.poseTFy -_pGoal_pose.goal_positionY));
                    if(m_distance < _pDynamic_param.MAX_Linear_velocity_origin*2){
                        float m_speed = _pDynamic_param.MAX_Linear_velocity_origin*(m_distance/2);
                        if(m_speed<0.01) m_speed= 0.01;
                        if(m_distance<0.1) m_speed= 0.0;
                        m_theta = m_speed == 0.0? 0.12: 0.06;

                        Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_x", 0.005);
                        Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_x_backwards", 0.0);
                        _pDynamic_param.MAX_Linear_velocity = m_speed;
                    }else{
                        Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_x", 0.005);
                        Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_x_backwards", 0.01);
                        _pDynamic_param.MAX_Linear_velocity = 0.01;
                    }
                }
                Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_theta", m_theta); 
        ...
        else
        {
            if(!m_flag_Dynamic_Teblocalplan_minor_update)
            {
                ...
                if(_pGoal_pose.goal_positionX != 0.0 || _pGoal_pose.goal_positionY != 0.0){
                    ...
                    if(m_distance < _pDynamic_param.MAX_Linear_velocity_origin*2){
                        float m_speed = _pDynamic_param.MAX_Linear_velocity_origin*(m_distance/2);
                        if(m_speed<0.01) m_speed= 0.0;
                        if(m_distance<0.1) m_speed= 0.0;
                        m_theta = m_speed == 0.0? 0.1: 0.04;

                        Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_x", m_speed);
                        Dynamic_reconfigure_Teb_Set_DoubleParam("max_vel_x_backwards", 0.0);
                        _pDynamic_param.MAX_Linear_velocity = m_speed;
                    }else{
...
void setDockGoal(move_base_msgs::MoveBaseActionGoal& goal)
{
    if(_pFlag_Value.m_bFlag_dock) return;
    ros::Time now = ros::Time::now();

    goal.header.frame_id="TETRA_NS/usb_cam";
    goal.header.stamp=now;
    goal.goal_id.stamp = now;
    goal.goal.target_pose.header.stamp = now;
    goal.goal.target_pose.header.frame_id = "TETRA_NS/usb_cam";
    goal.goal.target_pose.pose.position.x = 0.0;
    goal.goal.target_pose.pose.position.y = 0.0;
    goal.goal.target_pose.pose.position.z = 1.2;
    goal.goal.target_pose.pose.orientation.x = 1.0;
    ...
    service_pub.publish(goal);
    printf("setDockGoal call: %.5f, %.5f !!\n", 0.0, 0.9);
    _pFlag_Value.m_bFlag_pub = true;
    _pFlag_Value.m_bFlag_dock= true;
}
...
bool Depart_Station2Move()
{
    ...
    {
        if(_pAR_tag_pose.m_transform_pose_x <= 1.0) 
        {
				  ...
				  tetraDS_service::gotolocation::Response &res)
{
    ...
    if((_pRobot_Status.m_iCallback_Charging_status <= 1 && (!ex_bMissionDocking_Flag && !_pFlag_Value.m_bumperhit_flag)) && (_pAR_tag_pose.m_iAR_tag_id == -1 || ((_pAR_tag_pose.m_iAR_tag_id != _pRobot_Status.HOME_ID && _pAR_tag_pose.m_transform_pose_x >= 0.65) || _pAR_tag_pose.m_transform_pose_x >= 0.6))) //Nomal
    {
    ...
    {
        if(_pAR_tag_pose.m_iAR_tag_id == _pRobot_Status.HOME_ID)
        {
    ...
    while(1)
    {
        switch(ex_iDocking_CommandMode)
        {
            ...
            case 7:
                ...
                if(_pAR_tag_pose.m_fAR_tag_pitch <= 0.0174533 && _pAR_tag_pose.m_fAR_tag_pitch >= -0.0174533) //+- 1.0deg
                {
                    ex_iDocking_CommandMode = 2;
                }
                else if(_pAR_tag_pose.m_iAR_tag_id == _pAR_tag_pose.m_iSelect_AR_tag_id)
                {
                    setDockGoal(goal);
                }
                else if(!_pFlag_Value.m_bFlag_dock)
                {
                    geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
                    printf("No Marker, Rotation Movement 3!! \n");
                    cmd->angular.z = Rotation_Movement(); //0.1;
                    cmdpub_.publish(cmd);
                    ...
                    if(m_iNoMarker_cnt > 4000) //retry timeout!!
                    {
                        ...
                        cmd->angular.z = 0.0;
                        cmdpub_.publish(cmd);
                        printf("DockingStation scan Fail 3!! \n");
                        ex_iDocking_CommandMode = 9;
        ...
        if(_pRobot_Status.m_iCallback_Charging_status == 1)
        {
            ...
            if(m_bActive_map_check)
            {
                ...
                if(_pGoal_pose.goal_positionX != 0.0 || _pGoal_pose.goal_positionY != 0.0){
                    float m_distance= sqrt((_pTF_pose.poseTFx -_pGoal_pose.goal_positionX) * (_pTF_pose.poseTFx -_pGoal_pose.goal_positionX) + (_pTF_pose.poseTFy -_pGoal_pose.goal_positionY) * (_pTF_pose.poseTFy -_pGoal_pose.goal_positionY));
                    if(m_distance < _pDynamic_param.MAX_Linear_velocity_origin*2){
                        float m_speed = _pDynamic_param.MAX_Linear_velocity_origin*m_distance;
                        if(m_speed<0.005) m_speed= 0.005;
