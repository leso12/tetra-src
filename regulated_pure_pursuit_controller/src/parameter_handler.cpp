  ...
  const double costmap_size_x)
{
  ...
  // ---- Defaults 강화를 통해 현장 안전성 ↑ ---------------------------------- [MOD]
  private_node_->param<bool>("use_cost_regulated_linear_velocity_scaling", params_.use_cost_regulated_linear_velocity_scaling, true);
  ...
  private_node_->param<double>("regulated_linear_scaling_min_speed", params_.regulated_linear_scaling_min_speed, 0.12); // 0.1→0.12  [MOD]
  ...
  private_node_->param<bool>("use_collision_detection", params_.use_collision_detection, true); // false→true [MOD]
